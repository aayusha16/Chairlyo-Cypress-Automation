...
describe("Chairlyo Branch Add Session Functionality", () => {
  ...
  it("Verify adding service to customer", () => {
    ...
    cy.xpath(selector.session_customer_option).should("be.visible").click();
    cy.xpath(selector.session_next_button).should("be.visible").click();
    cy.xpath(selector.add_first_service).scrollIntoView().click({ force: true });
    cy.xpath(selector.service_search).should("be.visible").type("Aurelia Hammond");
    cy.xpath(selector.service_option).filter(":visible").should("have.length", 1).click();
    cy.xpath(selector.done_button).should("be.visible").click();
    cy.xpath(selector.select_staff).should("be.visible").click();
    cy.xpath(selector.staff_option1).filter(":visible").first().click();
    cy.xpath(selector.add_to_session_button).filter(":visible").last().should("be.visible").click();
  });
